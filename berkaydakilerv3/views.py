...
def kurs_profil(id):
    ...
    result = db.engine.execute("SELECT icerik,Kurs_yorum.date,puan,first_name,last_name,image_file FROM User, Kurs_yorum WHERE Kurs_yorum.user_id = User.id AND Kurs_yorum.kurs_id = 1 GROUP BY User.id ORDER BY Kurs_yorum.date ASC")
    
    return render_template("kurs_profil.html", user=current_user, kurs = kurs, egitmen = egitmen, dersler = dersler, yorumlar = result)
...
def ders(id):
    if request.method == 'POST':
        yorum_id = request.form.get('yorum')
...
def ders_yoruma_cvp(id):
    ...
    egitmen_id = kurs.egitmen_id
    egitmen = User.query.get_or_404(egitmen_id) 
    return render_template("ders_yoruma_cvp.html", user=current_user, ders = ders, kurs = kurs, egitmen = egitmen, yorum_id = id)
