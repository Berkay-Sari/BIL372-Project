...
def kurs_profil(id):
    if request.method == 'POST':
        ...
        if not rating:
            ...
            x = KursYorum.query.filter_by(user_id = current_user.id,kurs_id=id).first()
            if x:
                x.icerik = yorum
                x.puan = rating
            else:
...
@views.route('/kurs_ayarlari/<int:id>', methods=['GET', 'POST'])
@login_required
def kurs_ayarla(id):
    ...
    if request.method == 'POST':
        ...
        uzun_aciklama = request.form.get('uzun_açıklama')
        img = request.files['image']
        filename = kurs.image_file
        if img and allowed_file(img.filename):
            filename = save_picture2(img)
        
        if isim:
            x = Kurs.query.filter_by(isim=isim).first()
            if x and x.isim != kurs.isim:
                flash('Aynı isimde bir kurs bulunmaktadır.', category='error')
            else:
                kurs.isim = isim
        if kategori:
            kurs.kategori = kategori
        if kisa_aciklama:
            kurs.kisa_aciklama = kisa_aciklama
        if uzun_aciklama:
            kurs.uzun_aciklama = uzun_aciklama
        db.session.commit()
        return redirect('/kurs_profil/' + str(id))
...
def profil(id):
    ...
    if user.role == 1:
        ...
        verilen_kurslar = db.engine.execute("SELECT Kurs.image_file AS img, Kurs.id,isim,kategori, kisa_aciklama, Kurs.date AS kurs_date FROM Kurs WHERE Kurs.egitmen_id ="+str(current_user.id))
