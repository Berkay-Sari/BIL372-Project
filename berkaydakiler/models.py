...
class KursYorum(db.Model):
    ...
    date = db.Column(db.DateTime(timezone=True), default=func.now())
    puan = db.Column(db.Float, CheckConstraint('puan>0 AND puan<5'))
...
class DersYorum(db.Model):   
    ...
    parent_yorum = db.Column(db.Integer, db.ForeignKey('ders_yorum.id'), nullable = True)
    children = db.relationship("DersYorum")
...
class FileContent(db.Model):
    ...
    def __repr__(self):
        return f'Pic Name: {self.name} Data: {self.data} text: {self.text} created on: {self.date} location: {self.location}'
